...
`;
  ...
  const queryParams = [
    ...
    filters.currentCount?.min ?? null, filters.currentCount?.min ?? null,
    filters.currentCount?.max ?? null, filters.currentCount?.max ?? null,
  ...
  const totalRows = countResult.length
...
export const getFilteredReport = async (
  ...
  (
      ...
      WHERE 
          ...
          AND (n.days_90_plus >= ? OR ? IS NULL)
          AND (
              (COALESCE(n.days_30_to_60, 0) + COALESCE(n.days_60_to_90, 0) + COALESCE(n.days_90_plus, 0)) / NULLIF(m.building_meter_count, 0)
          )*100 BETWEEN ? AND ?
  )
  UNION
  (
  ...
  const queryParams = [
    ...
    filters.days90Plus?.min ?? null, filters.days90Plus?.min ?? null,
    filters.nonCommCount?.min ?? null, filters.nonCommCount?.max ?? null,
